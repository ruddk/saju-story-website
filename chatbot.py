...
class SajuChatbot:
    ...
    def _simple_response(self, message: str) -> str:
        """LangChain이 없을 때 사용하는 간단한 응답 시스템"""
        message_lower = message.lower()
        
        # 상담 비용 관련
        if any(keyword in message_lower for keyword in ['비용', '가격', '얼마', '요금']):
            return "60분 상담은 50,000원입니다. 상담 방법은 방문 상담, 전화 상담, 영상 상담이 가능합니다."
        
        # 영업시간 관련
        if any(keyword in message_lower for keyword in ['영업시간', '운영시간', '시간', '휴무']):
            return "화요일-일요일 10:00-19:00에 운영하며, 월요일은 휴무입니다. 점심시간은 12:00-13:00입니다."
        
        # 위치 관련
        if any(keyword in message_lower for keyword in ['위치', '주소', '어디', '찾아가']):
            return "서울시 강남구 테헤란로 123 사주도서관 빌딩 2층에 위치해 있습니다. 2호선, 신분당선 강남역 3번 출구에서 도보 5분 거리입니다."
        
        # 연락처 관련
        if any(keyword in message_lower for keyword in ['연락처', '전화', '번호', '카카오']):
            return "전화: 010-1234-5678, 이메일: saju@library.com, 카카오톡: @수목선생사주도서관으로 연락주세요."
        
        # 예약 관련
        if any(keyword in message_lower for keyword in ['예약', '상담받고', '예약하고']):
            available_times = self.get_available_times()
            if available_times:
                return f"예약을 도와드리겠습니다! 현재 예약 가능한 시간은 다음과 같습니다:\n\n{chr(10).join(available_times[:5])}\n\n원하시는 날짜와 시간을 알려주세요."
            else:
                return "죄송합니다. 현재 예약 가능한 시간이 없습니다. 다른 날짜를 문의해주세요."
        
        # 기본 응답
        return "안녕하세요! 수목선생의 사주이야기 상담도우미입니다. 상담 예약, 비용, 위치, 영업시간 등에 대해 문의해주세요."
